...
class SubgoalMiniGridEnv(MiniGridEnv):
    ...
    def step(self, action):
        ...
        # When there was no movement of the agent, gives some penalty
        if not np.allclose(self.agent_pos, fwd_pos):
            reward -= 1 / (self.max_steps * 4)
...
class OptRewardMiniGridEnv(SubgoalMiniGridEnv):
    ...
    def reward_fn(self):
        ...
        if (self.horizontal and self.agent_pos[0] > self.subgoal_pos[0]) \
            ...
            dist = self.manhattan_distance(self.agent_pos, self.subgoal_pos) + self.manhattan_distance(self.subgoal_pos,
                                                                                                       target_pos)
